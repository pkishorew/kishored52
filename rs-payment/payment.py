...
def pay(id):
    ...
    # check user exists
    try:
        req = requests.get('http://user:8080/check/{id}'.format(user=USER, port=USER_PORT, id=id))
    ...
    if not anonymous_user:
        try:
            req = requests.post('http://user:8080/order/{id}'.format(user=USER, port=USER_PORT, id=id),
    ...
    # delete cart
    try:
        req = requests.delete('http://cart:8080/cart/{id}'.format(cart=CART, port=CART_PORT,id=id));
